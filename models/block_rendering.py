...
def block_render(model, rays_o, rays_d, gating_code, warmup=False, **kwargs):
    """
    Render rays by
    ...
    # if warmup:
    depth_results = torch.zeros(rays_o.shape[0]).to(device)
    ...
    for i in range(model.size):
        ...
        depth_results += results['depth'] * gating_code[:,i]
