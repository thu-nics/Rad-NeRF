...
def ml_render(model, gating_net, rays_o, rays_d, imgs_d, warmup=False, **kwargs):
    """
    Render rays by
    ...
    rays_o = rays_o.contiguous(); rays_d = rays_d.contiguous(); imgs_d = imgs_d.contiguous()
    kwargs['return_gate'] = False

    # gating code aggregation
    if gating_net.type == 'image':
        rays_data = torch.cat((rays_o, imgs_d), 1)
        gating_code, gating_importance, gating_indices = gating_net(rays_data, warmup)
    elif gating_net.type == 'ray':
        rays_data = torch.cat((rays_o, rays_d), 1)
        gating_code, gating_importance, gating_indices = gating_net(rays_data, warmup)
    
        
    # only reserve rgb & depth results
    # if warmup:
    depth_results = torch.zeros(rays_o.shape[0], model.size).to(device)
    rgb_results = torch.zeros(rays_o.shape).to(device)
    opacity_results = torch.zeros(rays_o.shape[0]).to(device)
    ...
    for i in range(model.size):
        ...
        results = render_func(model, rays_o, rays_d, hits_t, i, **kwargs)
        ...
        independent_rgbs.append(results['rgb'])
        rgb_results += results['rgb'] * gating_code[:,i][:,None]
        depth_results[:,i] = results['depth']
        opacity_results += results['opacity'] * gating_code[:,i]
    
    # import ipdb; ipdb.set_trace()
    
    output_results['rgb'] = rgb_results
    output_results['independent_rgbs'] = independent_rgbs
    output_results['depth'] = depth_results
    output_results['opacity'] = opacity_results
    output_results['gating_code'] = gating_code
    output_results['gating_importance'] = gating_importance
    return output_results


@torch.no_grad()
def __render_rays_test(model, rays_o, rays_d, hits_t, ind, **kwargs):
    """
    Render rays by
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        density_bitfield = getattr(model, 'density_bitfield_{}'.format(ind))
        xyzs, dirs, deltas, ts, N_eff_samples = \
            vren.raymarching_test(rays_o, rays_d, hits_t[:, 0], alive_indices,
                                  density_bitfield, model.cascades,
        ...
        sigmas[valid_mask], _rgbs = model(xyzs[valid_mask], dirs[valid_mask], ind, **kwargs)
...
def __render_rays_train(model, rays_o, rays_d, hits_t, ind, **kwargs):
    """
    Render rays by
    ...
    results['deltas'], results['ts'], results['rm_samples']) = \
        RayMarcher.apply(
            rays_o, rays_d, hits_t[:, 0], density_bitfield,
    ...
    sigmas, rgbs = model(xyzs, dirs, ind, **kwargs)
