...
def render(model, rays_o, rays_d, **kwargs):
    ...
    if kwargs['moe_training']:
        kwargs['return_gate'] = True
...
@torch.cuda.amp.autocast()
def moe_render(model_zoo, gating_net, rays_o, rays_d, warmup=False, **kwargs):
    """
    Render rays by
    1. Compute the intersection of the rays with the scene bounding box
    ...
    if gating_net.type == 'position':
        gating_code, gating_importance, gating_indices = gating_net(rays_o, warmup)
    ...
    # if warmup:
    depth_results = torch.zeros(rays_o.shape[0], model_zoo.size).to(device)
    rgb_results = torch.zeros(rays_o.shape).to(device)
    opacity_results = torch.zeros(rays_o.shape[0]).to(device)

    for i in range(model_zoo.size):
        model = getattr(model_zoo, 'model_{}'.format(i))
        # model = model_zoo.models[i]
        _, hits_t, _ = \
            RayAABBIntersector.apply(rays_o, rays_d, model.center, model.half_size, 1)
        hits_t[(hits_t[:, 0, 0]>=0)&(hits_t[:, 0, 0]<NEAR_DISTANCE), 0, 0] = NEAR_DISTANCE
    ...
    results = {}
    results['rgb'] = rgb_results
    results['depth'] = depth_results
    results['opacity'] = opacity_results
    results['gating_code'] = gating_code
    results['gating_importance'] = gating_importance
    return results
...
def __render_rays_test(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        if kwargs['moe_training'] and kwargs['return_gate']:
            sigmas[valid_mask], _rgbs, _gate = model(xyzs[valid_mask], dirs[valid_mask], **kwargs)
        else:
...
def __render_rays_train(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    if kwargs['moe_training'] and kwargs['return_gate']:
        sigmas, rgbs, gate_results = model(xyzs, dirs, **kwargs)
        results['gating_code'] = gate_results['code']
        results['gating_importance'] = gate_results['importance']
    else:
